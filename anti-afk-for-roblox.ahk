...
CheckingAdministrator() {
    if (tempBlockInput && !A_IsAdmin) {
        MsgBox("Cuz you have turned on tempBlockInput feature so please run script with 'Run as administrator'.`n`nTo do that:`n1. Right click on this script`n2. Click 'Run as administrator'", "Are you goldfish?")
        ExitApp
    }
}
...
CheckingSaveValueForEXE(*) {
    ...
    if A_IsCompiled {
        if !FileExist(A_Temp "/roblox-anti-afk-setttings.ini") {
            ...
            RobloxPlatformDescription.OnEvent("Click", (ctrl, info) => ShowDescription(ctrl, info, "RobloxPlatformDescription"))
...
ShowDescription(ctrl, info, i) {
    ...
    if (i == "CheckTimeDescription") {
        ...
        ToolTip("Change this to the button you want to press when switching tabs`nSearch codenames for button: https://www.autohotkey.com/docs/v2/KeyList.htm", mouse_desc_x + 10, mouse_desc_y - 20, 1)
    } else if (i == "RobloxPlatformDescription") {
        ToolTip("Roblox Platform of your roblox`n'MS' mean Microsoft Store version (Roblox download from Microsoft Store)`n'WEB' mean Offical version (Roblox download from offical website roblox.com)", mouse_desc_x + 10, mouse_desc_y - 20, 1)
    } else if (i == "BlockInputDescription") {
        ToolTip("Block any input from your Keyboard and Mouse when checking? and then it will unblock`n1 if argee`n0 if disargee`nThis feature only work if you run script with 'Run as administrator' (even with option 1)", mouse_desc_x + 10, mouse_desc_y - 20, 1)
    } else if (i == "GameIgnoreBIDescription") {
        ToolTip("This will ignore tempBlockInput feature when you activating roblox but still send button to prevent afk`nIt will not interrupt your experience when you are PvP or when you are gaming on Roblox`n1 if argee`n0 if disargee", mouse_desc_x + 10, mouse_desc_y - 20, 1)
    } else if (i == "MoveMouseDescription") {
        ToolTip("Move mouse to Roblox windows when checking? and then it will go back`n1 if argee`n0 if disagree", mouse_desc_x + 10, mouse_desc_y - 20, 1)
    }
...
HideDescription() {
    ...
    if ((newx - mouse_desc_x > 15 || newx - mouse_desc_x < -15) || (newy - mouse_desc_y > 15 || newy - mouse_desc_y < -15)) {
        ToolTip(, , , 1)
        SetTimer(, 0)
    }
}
...
SaveValue(*) {
    ...
    ToolTip("You have been changed the settings`nYou need to restart the app to apply new changes", appX - 885, appY - 140, 2)
